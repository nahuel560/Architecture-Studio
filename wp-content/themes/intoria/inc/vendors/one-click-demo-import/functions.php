...
function intoria_ocdi_import_files() {
    $demos = array();
    $demos[] = array(
        ...
        'local_import_redux'           => array(
            array(
                ...
                'option_name' => 'intoria_theme_options',
            ),
        ),
        'import_preview_image_url'     => trailingslashit( get_template_directory_uri() ) . 'screenshot.png',
        'import_notice'                => esc_html__( 'Import process may take 5-10 minutes. If you facing any issues please contact our support.', 'intoria' ),
        'preview_url'                  => 'https://www.demoapus-wp1.com/intoria/',
    );
    return apply_filters( 'intoria_ocdi_files_args', $demos );
}
add_filter( 'pt-ocdi/import_files', 'intoria_ocdi_import_files' );

function intoria_ocdi_after_import_setup( $selected_import ) {
    ...
    if ( file_exists($file) ) {
        intoria_ocdi_import_settings($file);
    }
    ...
    if ( intoria_is_revslider_activated() ) {
        ...
        $slider_array = array(
            ...
            trailingslashit( get_template_directory() ) . 'inc/vendors/one-click-demo-import/data/slideshow-home-2.zip',
            trailingslashit( get_template_directory() ) . 'inc/vendors/one-click-demo-import/data/slideshow-home-3.zip',
            trailingslashit( get_template_directory() ) . 'inc/vendors/one-click-demo-import/data/slideshow-video.zip',
        );
...
add_action( 'pt-ocdi/after_import', 'intoria_ocdi_after_import_setup' );

function intoria_ocdi_import_settings($file) {
    ...
    if ( !empty($datas['page_options']) ) {
        intoria_ocdi_import_page_options($datas['page_options']);
    }
    if ( !empty($datas['metadata']) ) {
        intoria_ocdi_import_some_metadatas($datas['metadata']);
    }
}

function intoria_ocdi_import_page_options($datas) {
    if ( $datas ) {
...
function intoria_ocdi_import_some_metadatas($datas) {
    if ( $datas ) {
...
function intoria_ocdi_before_widgets_import() {
...
add_action( 'pt-ocdi/before_widgets_import', 'intoria_ocdi_before_widgets_import' );